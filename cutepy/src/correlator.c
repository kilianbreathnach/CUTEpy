...
static inline int rt2bin(double rt2)
{
  int irt;
  ...
  if(logbin) {
    ...
    irt=(int)(sqrt(rt2)*i_rt_max*nb_rt);
  }
...
double angular_separation(double *pos1, double *pos2)
{
    double cross2, dot;;

    cross2 = pow((pos1[1] * pos2[2] - pos1[2] * pos2[1]), 2) +
             pow((pos1[2] * pos2[0] - pos1[0] * pos2[2]), 2) +
             pow((pos1[0] * pos2[1] - pos1[1] * pos2[0]), 2);
    dot = pos1[0] * pos2[0] + pos1[1] * pos2[1] + pos1[2] * pos2[2];

    return sqrt(cross2) / dot;
	  ...
	  for(iy=iymin;iy<=iymax;iy++) {
		...
		if(ip2>ip1) {
		  ...
		  for(jj=0;jj<np2;jj++) {
			...
			double rt2=r2-rl*rl;
			if(rt2<rt2_max) {
			  int irt=rt2bin(rt2);			  if((irt<nb_rt)&&(irt>=0)) {
#ifdef _WITH_WEIGHTS
			    hthread[irl+nb_rl*irt]+=pos1[3]*pos2[3];
#else //_WITH_WEIGHTS
			    hthread[irl+nb_rl*irt]++;
...
void auto_3d_ps_bf_collisions(int nbox_full,int *indices,Box3D *boxes,
		   histo_t *hh)
{
  //////
  // Monopole auto-correlator
	...
	for(jj=ii+1;jj<np1;jj++) {
	  ...
	  if(r2<r2_max) {
		...
		if((irt<nb_rt)&&(irt>=0)) {
		  ...
		  hthread[irl+nb_rl*irt]++;
#endif //_WITH_WEIGHTS
                  tan_angsep=angular_separation(pos1, pos2);
                  if (tan_angsep < 8.3495689524439491e-07)
                      hthread[irl+nb_rl*irt] *= COL_WT;
		}
	      }
	    }
	  }
