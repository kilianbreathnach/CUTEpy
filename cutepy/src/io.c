...
static void make_CF_cross(histo_t D1D2,histo_t D1R2,
...
void write_CF_cross(char *fname,
         	    histo_t *D1D2,histo_t *D1R2,
                    histo_t *R1D2,histo_t *R1R2,
        	    np_t sum_wd,np_t sum_wd2,
        	    np_t sum_wr,np_t sum_wr2,
       		    np_t sum_wd_2,np_t sum_wd2_2,
...
void read_run_params(char *fname)
{
  ...
  for(ii=0;ii<n_lin;ii++) {
    ...
    if(!strcmp(s1,"data_filename="))
      ...
      sprintf(fnameData_2,"%s",s2);
    else if(!strcmp(s1,"random_filename_2="))
      sprintf(fnameRandom_2,"%s",s2);
	...
	estimator=3;
      ...
      else if(!strcmp(s2,"3D_ps_collisions")) corr_type=8;
      else if(!strcmp(s2,"3D_ps_cross")) corr_type=9;
      else {
