...
void init_3D_params_cross(Catalog cat_dat_1,Catalog cat_ran_1,
                          Catalog cat_dat_2,Catalog cat_ran_2,int ctype)
{
  int ii;

  for(ii=0;ii<cat_dat_1.np;ii++) {
    double cth=cat_dat_1.cth[ii];
    double phi=cat_dat_1.phi[ii];
    double sth=sqrt(1-cth*cth);
    double rr=z2r(cat_dat_1.red[ii]);
    double x=rr*sth*cos(phi);
    double y=rr*sth*sin(phi);
    double z=rr*cth;
  ...
  for(ii=0;ii<cat_ran_1.np;ii++) {
    double cth=cat_ran_1.cth[ii];
    double phi=cat_ran_1.phi[ii];
    double sth=sqrt(1-cth*cth);
    double rr=z2r(cat_ran_1.red[ii]);
    double x=rr*sth*cos(phi);
    double y=rr*sth*sin(phi);
    double z=rr*cth;
  ...
  nside=optimal_nside(l_box_max,rmax,cat_dat_2.np);
...
int cross_followup(Catalog cat_dat_1,int ctype)
{
  int old_nside = n_side[0];
  if(n_side[1]>old_nside) old_nside=n_side[1];
  if(n_side[2]>old_nside) old_nside=n_side[2];
  ...
  nside=optimal_nside(l_box_max,rmax,cat_dat_1.np);
